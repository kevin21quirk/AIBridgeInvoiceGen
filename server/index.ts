...
app.post('/api/invoices', async (req, res) => {
  ...
  const upfrontAmount = requiresUpfrontPayment ? total * 0.5 : null;
  ...
  const { rows } = await pool.query(
    ...
    [invoiceNumber, clientId, type, status, JSON.stringify(items),
     subtotal, total, issueDate, dueDate, notes || null,
...
app.put('/api/invoices/:id', async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [clientId||null, type||null, items ? JSON.stringify(items) : null,
     status||null, subtotal??null, issueDate||null, dueDate||null,
